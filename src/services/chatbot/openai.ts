...
// Send message to our API endpoint with web search capabilities
export async function sendChatMessage(
  ...
  try {
    ...
    console.log('[Chatbot] Sending request to chat API...');
    const response = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        context: context.knowledgeContext ? [{
          ...
          snippet: context.knowledgeContext
        }] : []
      })
    });
    ...
    if (!response.ok) {
      ...
      console.error('[Chatbot] Chat API error:', errorData);
    ...
    // Fallback to local QA system if API fails
