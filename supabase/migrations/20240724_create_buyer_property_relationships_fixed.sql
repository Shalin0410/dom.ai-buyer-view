...
DROP POLICY IF EXISTS "Buyers can view available properties" ON properties;
CREATE POLICY "Buyers can view available properties" ON properties
  FOR SELECT USING (
    -- Always allow if buyer has a relationship with this property
    EXISTS (
      SELECT 1 FROM buyer_properties 
    ...
    OR
    -- Allow if property is available (not in escrow by someone else)
    (
      overall_status = 'available' 
      AND NOT EXISTS (
        SELECT 1 FROM buyer_properties 
        WHERE buyer_properties.property_id = properties.id 
        AND buyer_properties.status IN ('in_escrow', 'closed')
        AND buyer_properties.buyer_id != auth.uid()
      )
    )
  );
